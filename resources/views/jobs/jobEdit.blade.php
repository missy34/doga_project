...
use App\Job;
$id=session()->get('id');
$jobs = Job::where('client_id', $id)->where('job_status', 1)->first();//募集中
...
@section('content')
<form action="{{ url('jobs/jobNew/'.$job->id)}}" method="POST" class="" enctype="multipart/form-data">
    {{ csrf_field() }}
    <div class="wraps">
        ...
        <div class="wrap_profile">
            ...
            <input id="job_profile" type="file" name="image_url" value="@if($job->image_url) {{$job->image_url}} @else{{old('client_num_emp')}}@endif">
        ...
        <input type="hidden" name="client_id" value="{{$job->client_id}}" placeholder="">
